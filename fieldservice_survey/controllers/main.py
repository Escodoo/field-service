...
class FsmOrderAppraisalSurvey(Survey):
    def _get_user_input_domain(self, survey, line_filter_domain, **post):
        ...
        try:
            ...
            raise AccessDenied("Invalid FSM Order ID")
        ...
        if user.has_group('fieldservice_survey.group_fsm_appraisal_user'):
            return expression.AND([[('fsm_order_id', '=', fsm_order.id)], user_input_domain])

        if partner.id in fsm_order.partner_feedback_ids.ids:
            return expression.AND([[FSM_ORDER_ID, '=', fsm_order.id], [FSM_ORDER_PERSON_ID, '=', person.id]], user_input_domain)

        raise AccessDenied("You do not have access to this FSM Order Appraisal Survey")
    ...
    def appraisal_survey_results(self, fsm_order_id, **post):
        ...
        if fsm_order.person_id.partner_id == request.env.user.partner_id:
            return request.render(
                ...
                {'status_code': 'Forbidden',
                 'status_message': "You don't have access to this survey related to your appraisal."})
        ...
        if user.has_group('fieldservice_survey.group_fsm_appraisal_user') or user.has_group('base.group_system'):
            survey_sudo = request.env['survey.user_input'].sudo().search([(FSM_ORDER_ID, '=', fsm_order.id)],
                                                                         limit=1).survey_id

        if partner.id in fsm_order.partner_feedback_ids.ids:
            answer = request.env["survey.user_input"].sudo().search(
                [(FSM_ORDER_ID, "=", fsm_order.id), (FSM_ORDER_PERSON_ID, '=', person.id)],
                limit=1,
            )
        ...
        if not survey_sudo:
            raise AccessDenied("No survey found for the given FSM Order")
