...
class FsmOrderSurveySurvey(Survey):
    ...
    def survey_results(self, fsm_order_id, **post):
        ...
        if not survey_sudo.user_input_ids or not [input_id.id for input_id in survey_sudo.user_input_ids if input_id.state != 'new']:
        ...
        return request.render(result_template,
                                      {'survey': survey_sudo,
                                       'survey_dict': self.prepare_result_dict(survey_sudo, current_filters),
