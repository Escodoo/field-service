...
class FsmOrderAppraisalAskFeedback(models.TransientModel):
    _name = "fsm.order.appraisal.ask.feedback"
    ...
    def default_get(self, fields):
        ...
        result = super(FsmOrderAppraisalAskFeedback, self).default_get(fields)
        fsm_order = self.env["fsm.order"].browse(
            result.get("fsm_order_id")
        ) or self.env["fsm.order"].browse(self._context.get("active_ids", []))
        if (
            "survey_template_id" in fields
            and fsm_order
            and not result.get("survey_template_id")
        ):
            result[
                "survey_template_id"
            ] = fsm_order.company_id.fsm_order_appraisal_survey_template_id.id
        return result

    fsm_order_id = fields.Many2one(
        "fsm.order",
        default=lambda self: self.env.context.get("active_id", None),
        string="Appraisal Field Service Order",
    )
    fsm_order_person_id = fields.Many2one(
        related="fsm_order_id.person_id", string="Appraisal Person}"
    )
    subject = fields.Char(
    ...
    template_id = fields.Many2one(
        ...
        "Use template",
        index=True,
        domain="[('model', '=', 'fsm.order.appraisal.ask.feedback')]",
        default=lambda self: self.env.ref(
            "fieldservice_survey.mail_template_fsm_order_appraisal_ask_feedback",
    ...
    deadline = fields.Date(
        string="Answer Deadline", required=True, default=_get_default_deadline
    )
    ...
    def _prepare_survey_anwers(self, recipients):
    ...
    def _send_mail(self, answer):
        ...
        ctx = {
            "fsm_order_name": self.fsm_order_id.name,
        }
        ...
        try:
            ...
            _logger.warning(
                "QWeb template mail.mail_notification_light not found when "
                "sending appraisal feedback mails. Sending without layouting."
            )
        else:
            template_ctx = {
                ...
                ._get("fsm.order.appraisal.ask.feedback")
                .display_name,
    ...
    def action_send(self):
        ...
        answers = self._prepare_survey_anwers(recipients)
        answers.sudo().write({"fsm_order_id": self.fsm_order_id.id})
        answers.sudo().write({"fsm_order_person_id": self.fsm_order_person_id.id})
        ...
        for person in self.recipient_ids.filtered(lambda e: e.user_id):
            ...
            self.fsm_order_id.with_context(
                ...
                note=_(
                    "An appraisal feedback was requested. Please take time to fill "
        ...
        self.fsm_order_id.partner_feedback_ids |= self.recipient_ids
